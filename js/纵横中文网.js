...
function HelloShit(i, t, e, r) {
 ...
 function hzasieckses(i, t, e, r) {
     var n, s, a, o, g, d, u, l, c, b = new Array,
         h = t.length,
         f = "";
 ...
 function biDivideModulo(i, t) {
     var e, r, n = biNumBits(i),
         s = biNumBits(t),
         a = t.isNeg;
     if (n < s) return i.isNeg ? (e = biCopy(bigOne),
     e.isNeg = !t.isNeg,
     i.isNeg = !1,
     t.isNeg = !1,
     r = biSubtract(t, i),
     ...
     t = biShiftLeft(t, 1), ++g, ++s,
     o = Math.ceil(s / bitsPerDigit) - 1;
     r = biShiftLeft(r, g),
     n += g;
     for (var d = Math.ceil(n / bitsPerDigit) - 1, u = biMultiplyByRadixPower(t, d - o); - 1 != biCompare(r, u);)++e.digits[d - o],
     r = biSubtract(r, u);
     for (var l = d; l > o; --l) {
         ...
         for (var p = e.digits[l - o - 1] * (f * biRadix + m), v = c * biRadixSquared + (b * biRadix + h); p > v;)--e.digits[l - o - 1],
         p = e.digits[l - o - 1] * (f * biRadix | m),
         v = c * biRadix * biRadix + (b * biRadix + h);
         (r = biSubtract(r, biMultiplyDigit(u = biMultiplyByRadixPower(t, l - o - 1), e.digits[l - o - 1]))).isNeg && (r = biAdd(r, u), --e.digits[l - o - 1])
     }
 ...
 (function() {
     function i(i, t) {
         if (void 0 != i && "" != i && "1" != n) {
             var e = (new Date).getTime(),
                 r = t ? "/prelogincheck.do" : "/preregcheck.do";
             $.ajax({
                 type: "post",
                 url: r,
                 data: {
                 ...
                 success: function(i) {
                     if (i && 1 == i.code) {
                         ...
                         $(".error-more").show().find(".changecapt").attr({
                             src: t,
                             "data-src": t
                         }),
                         o = !0
     ...
     function t(i) {
         i = i || "出错啦！",
         $(".login-tips").find(".error-tip").show().html(i)
     }
     ...
     $("#telphone,#yzm,#imgyzm2,#imgyzm").focus(function() {
         e()
     }),
     $(".login-style").find("span").click(function() {
         if (!$(this).hasClass("login-s-active")) {
             if ($(".error-more").hide(),
             $(this).hasClass("login-s-msg")) return $(".login-box").eq(0).show().siblings(".login-box").hide(),
             $(this).addClass("login-s-active").siblings().removeClass("login-s-active"),
             e(),
             void $("#telphone").blur();
             $(this).hasClass("login-s-user") && ($(".login-box").eq(1).show().siblings(".login-box").hide(),
             $(this).addClass("login-s-active").siblings().removeClass("login-s-active"),
             $("#username").blur(),
             e())
         }
     ...
     $(".checkbox").find("label").click(function() {
         $(this).find("span").hasClass("icon-checked") ? $(this).find("span").removeClass("icon-checked") : $(this).find("span").addClass("icon-checked")
     });
     var u;
     $(".btn-submit").click(function() {
         var r = {};
         if ($(this).hasClass("submit-phone")) {
             if (r.unam = $.trim($("#telphone").val()),
             r.pwd = $.trim($("#yzm").val()),
             r.capt = $.trim($("#imgyzm").val()),
                 "" == r.unam) return void t("手机号不能为空");
             if (!a.test(r.unam)) return void t("手机号格式错误");
             if ("" == r.pwd) return void t("短信验证码不能为空")
         } else {
             if (r.tye = 1,
             r.unam = $.trim($("#username").val()),
             r.pwd = $("#password").val(),
                 "1" == n ? (r.sessionid = s.sessionid,
             r.sig = s.sig,
             r.scene = NVC_Opt.scene,
             r.token = s.token) : r.capt = $.trim($("#imgyzm2").val()),
             s && 0 == s.ispass && "1" == n) return t("请先通过人机验证"),
             void($.browser.msie && !0 === $.browser.msie && parseInt($.browser.version) < 9 && t("您的浏览器可能不支持人机验证，请使用ie9以上版本或者Chrome"));
             if ("" == r.unam) return void t("用户名不能为空");
             if ("" == r.pwd) return void t("密码不能为空");
             if (o && "" == r.capt) return void t("验证码不能为空")
         }
         r.captkey = u,
         ...
         function(r) {
             ...
             } else if (!g) return void t("请先获取短信验证码");
             $.ajax({
                 type: "post",
                 url: "/dologinv2.do",
                 data: n,
                 ...
                 success: function(e) {
                     if (e) switch (e.code) {
                         case 1:
                             location.href = comeBackUrl,
                             $(".checkbox").eq(n.type).find(".icon-checked").size() > 0 ? 0 == n.tye && CookieUtil.setCookie({
                                 name: "loginphone",
                                 value: n.unam,
                                 expires: 31536e3
                             }) : 0 == n.tye && CookieUtil.setCookie({
                                 name: "loginphone",
                                 value: "",
                                 expires: -1
                             });
                         ...
                         case 2302:
                             ...
                             break;
                         case 2201:
                             t(r = e.msg || "验证码已经过期，请重新发送"),
                             $("#yzm").val(""),
                             i(n.unam, n.tye);
                             break;
                         default:
                             var r = e.msg || "出错啦，请您刷新后重试";
                             t(r),
                             i(n.unam, n.tye)
                     } else t("出现未知错误，请您重试!")
                 }
     ...
     $(".sentphonecapt").click(function() {
         ...
         if (i.hasClass("sentend")) t("发送太频繁啦，请您" + c + "秒后再试");
             ...
             var e = $.trim($("#telphone").val()),
                 r = $.trim($("#imgyzm").val());
             "" != e ? a.test(e) ? o && "" == r ? t("图片验证码不能为空") : d && $.ajax({
                 type: "post",
                 url: "/sendloginsms.do",
                 data: {
                     phone: e,
                 ...
                 success: function(e) {
                     if (e && 1 == e.code) i.html("发送成功(" + c + ")").addClass("sentend"),
                     g = !0,
                     l = setInterval(function() {
                         ...
                         0 == c && (clearInterval(l),
                         c = 60,
                         i.html("发送验证码").removeClass("sentend"))
                     }, 1e3),
                     u = e.data.captkey || "";
                     else if (e && 9002 == e.code) location.reload();
                     else {
                         g = !1;
                         t(e.msg || "发送手机验证码失败！")
                     }
     ...
     $(".login-box").find(".login-text").keydown(function(i) {
         13 == (i || window.event).keyCode && $(this).parents(".login-box").find(".btn-submit").click()
     })
 });
